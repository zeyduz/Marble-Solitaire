...
/**
 * Abstract class for the Model of the Marble Solitaire Game. This allows different versions of the
 * game to be plated without code duplication.
 */
public abstract class AbstractModel implements MarbleSolitaireModel {
  ...
  /**
   * Constructor that only takes in the size and sets the row and column at the center.
   *
   * @param size number of marbles in the top row
   * @throws IllegalArgumentException if the size is even or less than 3
   */
  public AbstractModel(int size) throws IllegalArgumentException {
    this(size, (size - 1) + ((size - 1) / 2), (size - 1) + ((size - 1) / 2));
  }
   ...
   */
  public AbstractModel(int sRow, int sCol) throws IllegalArgumentException {
    this(3, sRow, sCol);
  }

  /**
   * Constructor that takes in size, row, and column of the empty.
   *
   * @param size   number of marbles in the top row
   ...
   */
  public AbstractModel(int size, int row, int column) throws IllegalArgumentException {
    this.invalidSize(size);
    this.size = size;
    if (!(this.withinValidBounds(size, row, column))) {
  ...
  /**
   * Determines if the given size is invalid.
   *
   * @param size given size of board
   * @return whether the given side is invalid (even or less than three)
   */
   ...
   *
   * @param row row position of the empty slot
   * @param col column position of the empty slot
   */
  protected void makeBoard(int row, int col) {
    for (int i = 0; i < this.getBoardSize(); i++) {
      for (int j = 0; j < this.getBoardSize(); j++) {
        if (this.withinValidBounds(size, i, j) && (!(i == row && j == col))) {
   ...
   *
   * @param size arm thickness of the game
   * @param row  the row of the position
   * @param col  the column of the position
   * @return whether the position is valid or not, i.e., within bounds of the board
   */
   ...
   */
  protected boolean canHopOver(int marbleRow, int marbleCol, int emptyRow, int emptyCol) {
    if (marbleCol + 1 == 0 || marbleRow + 1 == 0 || emptyRow + 1 == 0 || emptyCol + 1 == 0) {
      marbleRow = 0;
      marbleCol = 0;
      emptyRow = 0;
      emptyCol = 0;
    }
    if (!(this.withinValidBounds(size, marbleRow, marbleCol)
            && this.withinValidBounds(size, emptyRow, emptyCol))) {
      return false;
    }
   ...
   * @throws IllegalStateException    when the game is over ( different cases for different boards)
   *
   */
  @Override
  public void move(int fromRow, int fromCol, int toRow, int toCol)
          throws IllegalArgumentException, IllegalStateException {
    if (this.withinValidBounds(size, toRow, toCol)
            && this.withinValidBounds(size, fromRow, fromCol)
